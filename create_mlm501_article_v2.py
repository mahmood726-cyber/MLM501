...
doc.add_paragraph('Figure 1. Entropy Landscape: Mapping MAFI vs. Heterogeneity (I²). This plot illustrates the relationship between evidence fragility and between-study inconsistency across 2,373 analyses. (Source: inst/extdata/cfa_entropy_landscape.png)')

doc.add_paragraph('Figure 2. Multi-Measure Landscape. Global map of evidence robustness across dichotomous (logOR), continuous (SMD/MD), and generic inverse-variance (GIV) measures. (Source: inst/extdata/cfa_multi_measure_landscape.png)')

doc.add_paragraph('Figure 3. Temporal Trends in Evidence Fragility. This chart demonstrates the sigmoidal "rotting" of evidence over time, highlighting the vulnerability of conclusions older than 12 years. (Source: inst/extdata/cfa_temporal_trends.png)')

doc.add_paragraph('Figure 4. Cardiovascular Pathway Frontier. Sensitivity threshold mapping for clinical benefit, demonstrating how diagnostic accuracy ripples through to treatment harms. (Source: inst/extdata/pathway_frontier_cvd.png)')

doc.add_paragraph('Figure 5. Distribution of Evidence Measures. Summary of the statistical metrics (logOR, SMD, GIV) covered in the global audit. (Source: inst/extdata/cfa_measure_distribution.png)')
